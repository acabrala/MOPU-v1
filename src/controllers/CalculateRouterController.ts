...
export class CalculateRouterController {

    private calculateRouterRepository: CalculateRouterRepository;
    private linhasTrilhosRepository: LinhasTrilhosRepository;

    constructor(calculateRouterRepository: CalculateRouterRepository,
        linhasTrilhosRepository: LinhasTrilhosRepository) {
        this.calculateRouterRepository = calculateRouterRepository
        this.linhasTrilhosRepository = linhasTrilhosRepository
    }

    calculateRouterNow = async (req, res) => {
        ...
        try {
            ...
            if ((router_is_bus == 'false') && (router_is_subway == 'false') && (router_is_train == 'false')) {
                way = await this.calculateRouterRepository.tracingRouter(req.params);
            } else if ((router_is_bus == 'false') && (router_is_subway == 'false') && (router_is_train == 'true')) {
                way = await this.calculateRouterRepository.tracingRouterTrain(req.params)
                trem = true;
            } else if ((router_is_bus == 'false') && (router_is_subway == 'true') && (router_is_train == 'false')) {
                way = await this.calculateRouterRepository.tracingRouterSubway(req.params)
                metro = true
            } else if ((router_is_bus == "true") && (router_is_subway == 'false') && (router_is_train == 'false')) {
                way = await this.calculateRouterRepository.tracingRouterBus(req.params)
            } else if ((router_is_bus == "true") && (router_is_subway == 'true') && (router_is_train == 'false')) {
                way = await this.calculateRouterRepository.trancingRouterBusAndSubway(req.params)
            } else if ((router_is_bus == "true") && (router_is_subway == 'false') && (router_is_train == 'true')) {
                way = await this.calculateRouterRepository.trancingRouterBusAndTrain(req.params)
            ...
            if (way) {
                ...
                for (let k = 0; k < way.data.routes.length; k++) {
                    ...
                    const linhasIndisponivel = await this.linhasTrilhosRepository.getLinesAvailable();
                    ...
                    if (c.length > 0) {
                        ...
                        if (tremIndisponivel == true && metroIndisponivel == false) {
                            ...
                            let metroRouter = await this.calculateRouterRepository.tracingRouterSubway(req.params)
                            ...
                            let trainRouter = await this.calculateRouterRepository.tracingRouterTrain(req.params)
                            ...
                            let busRouter = await this.calculateRouterRepository.tracingRouterBus(req.params)
